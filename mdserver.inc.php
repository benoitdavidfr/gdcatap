 ...
 *
 * Modif le 17/11/2023: Géo-IDE ne renvoit pas toujours 10 éléments même avant la fin
 */
...
class Cache {
  ...
  function __construct(string $harvest, string $cachedir, string $fileExt='.xml') {
    ...
    $cachedir = __DIR__."/harvests/$harvest/$cachedir";
    if (!is_dir(__DIR__."/harvests"))
      mkdir(__DIR__."/harvests");
    if (!is_dir(__DIR__."/harvests/$harvest"))
      mkdir(__DIR__."/harvests/$harvest");
    if (!is_dir($cachedir))
      mkdir($cachedir);
...
class Server {
  ...
  static function servers(): array {
    if (!isset(self::$servers))
      self::$servers = Yaml::parseFile(__DIR__.'/catalogs.yaml')['catalogs'];
  ...
  static function exists(string $serverId): ?array {
...
class CswServer {
  ...
  /** Liste des noms des paramètres avec majuscules/manuscules comme utilisé dans les requêtes */
  const PARAMS_LIST = [
    'SERVICE',
    'VERSION',
    'REQUEST',
  ...
  function __construct(string $serverId, string $harvest, string $cachedir) {
    ...
    $this->cache = new Cache($harvest, $cachedir);
  }
...
class MdServer implements Iterator {
  ...
  function __construct(string $serverId, string $harvest, string $cachedir, int $startPosition) {
    $this->server = new CswServer($serverId, $harvest, $cachedir);
  ...
  function current(): SimpleXMLElement {
    ...
    //print_r($this->records->csw_SearchResults);
  ...
  function next(): void {
    //echo "next()<br>\n";
    $this->currentPos++;
    if (($this->currentPos - $this->firstPos) >= $this->records->csw_SearchResults['numberOfRecordsReturned']) {
